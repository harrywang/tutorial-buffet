...
BASE_PATH = "data/images_background"
TRAIN_CLASS = 1200
...
def loader(path=None):
    ...
    np.save(f"./data/train_omniglot.npy", (np.array(train_images) * 255).astype(np.uint8))
    np.save(f"./data/test_omniglot.npy", (np.array(eval_images) * 255).astype(np.uint8))
